...
MODELS = [("", "Gemma 4 12B"), ("_qwen3-8b", "Qwen3 8B")]
N_BOOT = 4000
...
def interaction():
    ...
    for tag, label in MODELS:
        ...
        if m is None:
            sys.exit(f"missing result set for {label}")
        data[label] = m
        sides = s
    ...
    print("Per-language gap difference between models "
          f"({b} minus {a}):\n")
    print(f"{'lang':10} {'diff':>7} {'95% CI':>17} {'p':>8}")
    raw_p = {}
    per_lang = {}
    for l in langs:
        d = draws[(b, l)] - draws[(a, l)]
        lo, hi = np.percentile(d, [2.5, 97.5])
        p = max(2 * min((d <= 0).mean(), (d >= 0).mean()), 1 / N_BOOT)
        raw_p[l] = p
        per_lang[l] = {"diff": float(np.mean(d)), "ci": [float(lo), float(hi)],
                       "p": float(p)}
        print(f"{LANG_NAMES[l]:10} {np.mean(d):+7.2f} [{lo:+7.2f},{hi:+7.2f}] "
              f"{'<0.001' if p < 0.001 else f'{p:.3f}':>8}")

    for l, adj in analyze.holm(raw_p).items():
        per_lang[l]["p_holm"] = float(adj)
    ...
    rel = {m: draws[(m, "en")] - np.mean([draws[(m, l)] for l in others], axis=0)
           for m in labels}
    inter = rel[b] - rel[a]
    lo, hi = np.percentile(inter, [2.5, 97.5])
    p = max(2 * min((inter <= 0).mean(), (inter >= 0).mean()), 1 / N_BOOT)

    print(f"\nEnglish relative to the mean of the other six languages:")
    for m in labels:
        rlo, rhi = np.percentile(rel[m], [2.5, 97.5])
        print(f"  {m:14} {np.mean(rel[m]):+7.2f} [{rlo:+7.2f},{rhi:+7.2f}]")
    print(f"\n  INTERACTION ({b} minus {a}): {np.mean(inter):+.2f} "
          f"[{lo:+.2f},{hi:+.2f}], p {'<0.001' if p < 0.001 else f'= {p:.3f}'}")
    ...
    g = [float(np.mean(draws[(a, l)])) for l in langs]
    q = [float(np.mean(draws[(b, l)])) for l in langs]
    rho = spearmanr(g, q).statistic
    rhos = np.array([spearmanr([draws[(a, l)][i] for l in langs],
                               [draws[(b, l)][i] for l in langs]).statistic
                     for i in range(0, N_BOOT, 8)])
    rlo, rhi = np.percentile(rhos, [2.5, 97.5])
    print(f"\nCross-model agreement on the language ordering: "
          f"Spearman rho = {rho:+.2f} [{rlo:+.2f},{rhi:+.2f}]")

    return {"per_language": per_lang,
            "interaction": {"estimate": float(np.mean(inter)),
                            "ci": [float(lo), float(hi)], "p": float(p)},
            "profile_rho": {"estimate": float(rho),
